import {Grid, InputLabel, Paper, Box, Container, FormControl, Select, MenuItem, Typography, Button, ListItemIcon, ListItemText, SelectChangeEvent} from "@mui/material";
...
const SelectComponent = () => {
  const {t} = useTranslation();
  const [apiID, setApiID] = useLocalStorage<number>("api-ID", 10);
  const handleChange = (event: SelectChangeEvent<number>) => {setApiID(event.target.value as number);};
  return (
    <Paper>
      <Box p={2}>
